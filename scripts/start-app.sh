...
echo "Starting OpenNovel services"
echo "Agent:   http://127.0.0.1:${AGENT_PORT}"
echo "Backend: http://127.0.0.1:${BACKEND_PORT}"
echo "Run ID:  ${RUN_ID}"
echo "Logs:    ${LOG_DIR}/agent.app.${RUN_ID}.log, ${LOG_DIR}/backend.app.${RUN_ID}.log"
...
OPENNOVEL_RUN_ID="${RUN_ID}" "${AGENT_DIR}/.venv/bin/uvicorn" app.main:app \
  ...
  --host 127.0.0.1 \
  --port "${AGENT_PORT}" \
  > "${LOG_DIR}/agent.app.${RUN_ID}.log" 2>&1 &
AGENT_PID=$!
...
NOVEL_AGENT_BASE_URL="${AGENT_BASE_URL}" \
  ...
  > "${LOG_DIR}/backend.app.${RUN_ID}.log" 2>&1 &
BACKEND_PID=$!
